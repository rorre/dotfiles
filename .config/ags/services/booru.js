...
class BooruService extends Service {
    _baseUrl = 'https://e621.net/posts.json';
    _mode = 'e621';
    ...
    async fetch(msg) {
        ...
        Utils.fetch(`${APISERVICES[this._mode].endpoint}?${paramString}`, options)
            ...
            .then((dataString) => { // Store interesting stuff and emit
                ...
                parsedData = parsedData.posts || parsedData
                // console.log(parsedData)
                this._responses[newMessageId] = parsedData.map(obj => {
                    return {
                        aspect_ratio: (obj.file.width / obj.file.height) || (obj.width / obj.height),
                        id: obj.id,
                        tags: typeof obj.tags === 'string' ? obj.tags : Array.of(Object.values(obj.tags).map(x => x.join(' '))).join(' '),
                        rating: obj.rating,
                        is_nsfw: (obj.rating != 's'),
                        md5: obj.md5 || obj.file.md5,
                        preview_url: obj.preview?.url || obj.preview_url,
                        preview_width: obj.preview?.width || obj.preview_width,
                        preview_height: obj.preview?.height || obj.preview_height,
                        sample_url: obj.sample?.url || obj.sample_url,
                        sample_width: obj.sample?.width || obj.sample_width,
                        sample_height: obj.sample?.height || obj.sample_height,
                        file_url: obj.file?.url || obj.file_url,
                        file_ext: obj.file?.ext || obj.file_ext,
                        file_width: obj.file?.width || obj.file_width,
                        file_height: obj.file?.height || obj.file_height,
                        source: Array.isArray(obj.source) ? getWorkingImageSauce(obj.source[0]) : getWorkingImageSauce(obj.source),
                    }
                });
